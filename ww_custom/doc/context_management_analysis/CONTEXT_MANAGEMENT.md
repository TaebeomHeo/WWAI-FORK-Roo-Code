...
**Issue**: For large repositories (Monorepos), the "Visible Files" list and terminal logs from thousands of lines can consume a huge chunk of the Context Window _in every single turn_.

- **Impact**: Local LLMs (8k window) get "flooded" by file paths, triggering truncation almost immediately and losing track of the actual task.
